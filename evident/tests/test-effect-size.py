...
class TestEffectSize(TestCase):
    ...
    def test_effect_size_alpha(self):
        ...
        _effect_size.effect_size(mappings=self.mappings, alphas=alphas,
                                 ...
                                 permutations=None,
                                 alpha_method='mannwhitneyu',
                                 na_values=na_values,
                                 overwrite=True)

        # check effect size calculation for gender (two-group categorical)
        pfp = partial(join, output)
        gender_results = [('mannwhitneyu.alpha_pd.txt.Faith_PD'
                           '.mapping_gender.txt.Gender.pickle',
                           'mannwhitneyu.alpha_pd.txt.Faith_PD'
                           '.mappings.txt.Gender.pickle',
                           'mannwhitneyu.alpha_sn.txt.Shannon'
                           '.mapping_gender.txt.Gender.pickle',
                           'mannwhitneyu.alpha_sn.txt.Shannon'
                           '.mappings.txt.Gender.pickle',
                           'mannwhitneyu.alpha_otu.txt.Observed_OTUs'
                           '.mapping_gender.txt.Gender.pickle',
                           'mannwhitneyu.alpha_otu.txt.Observed_OTUs'
                           '.mappings.txt.Gender.pickle')]
        ...
        for gen_files in gender_results:
            ...
            with open(pfp(gender_pd), "rb") as file_gender_pd, \
                ...
                np.testing.assert_equal(len(results_gender_pd
                                            ['pairwise_comparisons']),
                                        len(results_gender_pd_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_gender_pd['pooled_pval'],
                                        results_gender_pd_g['pooled_pval'])

                np.testing.assert_equal(len(results_gender_sn
                                            ['pairwise_comparisons']),
                                        len(results_gender_sn_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_gender_sn['pooled_pval'],
                                        results_gender_sn_g['pooled_pval'])

                np.testing.assert_equal(len(results_gender_otu
                                            ['pairwise_comparisons']),
                                        len(results_gender_otu_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_gender_otu['pooled_pval'],
                                        results_gender_otu_g['pooled_pval'])

                np.testing.assert_array_less(results_gender_sn['pooled_pval'],
                                             results_gender_otu['pooled_pval'])
                np.testing.assert_array_less(results_gender_otu['pooled_pval'],
                                             results_gender_pd['pooled_pval'])

        # check effect size calculation for country (four-group categorical)
        country_results = [('mannwhitneyu.alpha_pd.txt.Faith_PD'
                            '.mapping_country.txt.Country.pickle',
                            'mannwhitneyu.alpha_pd.txt.Faith_PD'
                            '.mappings.txt.Country.pickle',
                            'mannwhitneyu.alpha_sn.txt.Shannon'
                            '.mapping_country.txt.Country.pickle',
                            'mannwhitneyu.alpha_sn.txt.Shannon'
                            '.mappings.txt.Country.pickle',
        ...
        for country_files in country_results:
            ...
            with open(pfp(country_pd), "rb") as file_country_pd, \
                ...
                np.testing.assert_equal(len(results_country_pd
                                            ['pairwise_comparisons']),
                                        len(results_country_pd_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_country_pd['pooled_pval'],
                                        results_country_pd_g['pooled_pval'])

                np.testing.assert_equal(len(results_country_sn
                                            ['pairwise_comparisons']),
                                        len(results_country_sn_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_country_sn['pooled_pval'],
                                        results_country_sn_g['pooled_pval'])

                np.testing.assert_equal(len(results_country_otu
                                            ['pairwise_comparisons']),
                                        len(results_country_otu_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_country_otu['pooled_pval'],
                                        results_country_otu_g['pooled_pval'])

                np.testing.assert_array_less(results_country_sn['pooled_pval'],
                                             results_country_otu
                                             ['pooled_pval'])
                np.testing.assert_array_less(results_country_otu
                                             ['pooled_pval'],
                                             results_country_pd
                                             ['pooled_pval'])

        # check effect size calculation for age (continous)
        age_results = [('mannwhitneyu.alpha_pd.txt.Faith_PD'
                        '.mapping_age.txt.Age.pickle',
                        'mannwhitneyu.alpha_pd.txt.Faith_PD'
                        '.mappings.txt.Age.pickle',
                        'mannwhitneyu.alpha_sn.txt.Shannon'
                        '.mapping_age.txt.Age.pickle',
                        'mannwhitneyu.alpha_sn.txt.Shannon'
                        '.mappings.txt.Age.pickle',
        ...
        for age_files in age_results:
            ...
            with open(pfp(age_pd), "rb") as file_age_pd, \
                ...
                np.testing.assert_equal(len(results_age_pd
                                            ['pairwise_comparisons']),
                                        len(results_age_pd_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_age_pd['pooled_pval'],
                                        results_age_pd_g['pooled_pval'])

                np.testing.assert_equal(len(results_age_sn
                                            ['pairwise_comparisons']),
                                        len(results_age_sn_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_age_sn['pooled_pval'],
                                        results_age_sn_g['pooled_pval'])

                np.testing.assert_equal(len(results_age_otu
                                            ['pairwise_comparisons']),
                                        len(results_age_otu_g
                                            ['pairwise_comparisons']))
                np.testing.assert_equal(results_age_otu['pooled_pval'],
                                        results_age_otu_g['pooled_pval'])

                np.testing.assert_array_less(results_age_sn['pooled_pval'],
                                             results_age_otu['pooled_pval'])
                np.testing.assert_array_less(results_age_otu['pooled_pval'],
